...
function workflowStepPlan(options: PrimaryWorkflowsOptions): string {
  ...
  if (options.enableCodebaseInvestigator && options.enableWriteTodosTool) {
    return `2. **Plan:** Build a coherent and grounded (based on the understanding in step 1) plan for how you intend to resolve the user's task. If the user's request implies a change but does not explicitly state it, **YOU MUST ASK** for confirmation before modifying code. If 'codebase_investigator' was used, do not ignore the output of the agent, you must use it as the foundation of your plan.${
      options.isHardcoreMode
        ...
        : ''
    } Break them down into smaller, manageable subtasks and use the \`${WRITE_TODOS_TOOL_NAME}\` tool to track your progress. Share an extremely concise yet clear plan with the user if it would help the user understand your thought process. As part of the plan, you should use an iterative development process that includes writing unit tests to verify your changes. Use output logs or debug statements as part of this process to arrive at a solution.`;
  }
  if (options.enableCodebaseInvestigator) {
    return `2. **Plan:** Build a coherent and grounded (based on the understanding in step 1) plan for how you intend to resolve the user's task. If the user's request implies a change but does not explicitly state it, **YOU MUST ASK** for confirmation before modifying code. If 'codebase_investigator' was used, do not ignore the output of the agent, you must use it as the foundation of your plan.${
      options.isHardcoreMode
  ...
  if (options.enableWriteTodosTool) {
    return `2. **Plan:** Build a coherent and grounded (based on the understanding in step 1) plan for how you intend to resolve the user's task. If the user's request implies a change but does not explicitly state it, **YOU MUST ASK** for confirmation before modifying code.${
      options.isHardcoreMode
        ? ' **CRITICAL: HARDCORE MODE IS ACTIVE.** You are in a high-transparency mode. For EVERY SINGLE tool call you make (including `grep_search`, `read_file`, `list_directory`, etc.), you MUST provide three additional mandatory parameters: `thought_what` (exact sub-task you are solving), `thought_why` (why this tool is the best choice right now), and `thought_how` (exactly how you will use it, including files/offsets). These parameters are REQUIRED by the tool schemas. If you omit them, the tool call will fail.'
        : ''
    } Break them down into smaller, manageable subtasks and use the \`${WRITE_TODOS_TOOL_NAME}\` tool to track your progress. Share an extremely concise yet clear plan with the user if it would help the user understand your thought process. As part of the plan, you should use an iterative development process that includes writing unit tests to verify your changes. Use output logs or debug statements as part of this process to arrive at a solution.`;
